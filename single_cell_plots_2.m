...
%% the age sort and death type selection are off
%%
clear all
% load NTS1_all_DT_20160314; 
% red = [1,0,0];
% green = [0,1,0];
% 
...
ax1 = gca;
set(ax1,'FontSize',10,'FontName','Arial','XLim',[0 x_max-1],'YLim',[0 6])
xlabel('Time (min)');
...
cell_title = ['Cell:',num2str(CCI),'RLS=',num2str(all_data_2(CCI).age),all_data(CCI).id];
%         
title(cell_title);
    end
end

heat_data=-1*ones(length(all_data),x_max);
for j=1:length(all_data)
    heat_data(j,1:length(all_data_2(j).traj_s))=all_data_2(j).traj_s;
end

figure;
h = imagesc(heat_data,[0.5 2.5]);
